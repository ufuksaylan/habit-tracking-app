<template>
  <header id="header">
    <div class="container-sm wrap">
      <div class="text">
        <h1>{{ headerText }}</h1>
      </div>
      <div class="image">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <path
            class="calender"
            d="M6.75 0a.75.75 0 0 1 .75.75V3h9V.75a.75.75 0 0 1 1.5 0V3h2.75c.966 0 1.75.784 1.75 1.75v16a1.75 1.75 0 0 1-1.75 1.75H3.25a1.75 1.75 0 0 1-1.75-1.75v-16C1.5 3.784 2.284 3 3.25 3H6V.75A.75.75 0 0 1 6.75 0ZM21 9.5H3v11.25c0 .138.112.25.25.25h17.5a.25.25 0 0 0 .25-.25Zm-17.75-5a.25.25 0 0 0-.25.25V8h18V4.75a.25.25 0 0 0-.25-.25Z"
          ></path>
        </svg>
      </div>
    </div>
  </header>
</template>

<script setup>
defineProps({
  headerText: { type: String, default: 'Today' }
});
</script>

<style>
#header svg {
  height: 2.2rem;
  transition: transform 0.3s ease;
}

#header .wrap {
  height: 100%;
  width: 100%;
  display: flex;
  justify-content: space-between;
  margin-top: 2rem;
}

#header svg path {
  fill: var(--text-color);
  height: 2.2rem;
  transition: fill 0.1s ease;
}

#header .image:hover > svg path {
  fill: var(--main-color);
}

#header .image:hover > svg {
  transform: scale(1.2);
}

.image:hover > .calender {
  fill: red; /* Change the color to red when hovering */
}
</style>
